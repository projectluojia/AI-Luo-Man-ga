...
- **生成文件是提交构件，禁止手工修改**：`proto/executor.proto` 或 `proto/runtime_host.proto` 变更后必须重新生成 Go 与 Python 产物并连同测试一起提交（`make generate`，CI 有漂移检查）。仓库内明确要求提交工件的 fixture 应提交由 `[build]` 生成的 wasm；零声明包作者只提交 guest 源码，由 `ailuo pack` 构建 wasm。
