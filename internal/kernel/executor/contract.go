...
// 本包，不认识任何具体实现或语言；agentv1 只是线协议的生成产物，本包是它
// 在内核侧的唯一边界。
package executor
...
type RunStream = agentv1.AgentRuntime_RunClient

// Client 是内核消费 AI 执行者所需的协议客户端面；agentv1 生成的客户端天然满足。
type Client interface {
	Run(ctx context.Context, opts ...grpc.CallOption) (RunStream, error)
	Health(ctx context.Context, in *agentv1.HealthRequest, opts ...grpc.CallOption) (*agentv1.HealthResponse, error)
}
