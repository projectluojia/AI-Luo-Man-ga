...
// 空闲教室 hosted 包端到端：真实 ailuo.toml 清单 + 真实 guest 源码现场编译 +
// packstore 快照与个人作用域日程。覆盖查询治理、日程生命周期（幂等 + 确认
// 门槛）与非权威快照拒绝，全部经真实 Dispatcher。
package classroomtest_test
...
func seedClassroom(t *testing.T, store packstore.Store) {
	t.Helper()
	now := time.Now().UTC()
	packageID := hostedtest.ManifestOf(t, "classroom").ID
	scope := packstore.Scope{AppID: packageID, PackageID: packageID, Namespace: hostedtest.Packages["classroom"].StorageNamespace}
...
func newClassroomDispatcher(t *testing.T) *runtime.Dispatcher {
	...
	hostedtest.RegisterHosted(t, reg, store, "classroom")
	return hostedtest.NewDispatcher(t, reg, hostedtest.ManifestOf(t, "classroom").ID, hostedtest.CapabilityIDs(t, "classroom"))
}
...
func TestHostedClassroomRoomsSearch(t *testing.T) {
	...
	ok, result, errText := invoke(t, d, hostedtest.ManifestOf(t, "classroom").ID, "classroom.rooms.search",
...
func TestHostedClassroomScheduleLifecycle(t *testing.T) {
	d := newClassroomDispatcher(t)
	appID := hostedtest.ManifestOf(t, "classroom").ID

	ok, result, errText := invoke(t, d, appID, "classroom.schedule.create",
	...
	// 列出日程（个人作用域注入 UserID）。
	ok, result, errText = invoke(t, d, appID, "classroom.schedule.list", `{}`, "", "")
	if !ok {
	...
	ok, _, errText = invoke(t, d, appID, "classroom.schedule.cancel",
	...
	ok, result, errText = invoke(t, d, appID, "classroom.schedule.cancel",
...
func TestHostedClassroomCreateUnknownRoom(t *testing.T) {
	d := newClassroomDispatcher(t)
	ok, result, errText := invoke(t, d, hostedtest.ManifestOf(t, "classroom").ID, "classroom.schedule.create",
...
func TestHostedClassroomGovernedSnapshotRejection(t *testing.T) {
	...
	hostedtest.RegisterHosted(t, reg, store, "classroom")
	d := hostedtest.NewDispatcher(t, reg, hostedtest.ManifestOf(t, "classroom").ID, hostedtest.CapabilityIDs(t, "classroom"))
	request := hostedtest.RequestContext(hostedtest.ManifestOf(t, "classroom").ID, "request-governed-rejection")
	_, invokeErr := d.InvokeCapability(t.Context(), request, "classroom.rooms.search", json.RawMessage(`{"date":"2026-09-07","campus_id":"c","period":1}`))
