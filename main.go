...
func loadConfig() (config, error) {
	...
	result := config{
		...
		pythonPath:         envOr("AILUO_PYTHON", "agent/.venv/bin/python"),
...
func validateSecretFile(path string) error {
	...
	if !info.Mode().IsRegular() || info.Mode().Perm()&0o077 != 0 || info.Size() <= 0 || info.Size() > 16<<10 ||
		...
		return errorsNew("AILUO_MODEL_API_KEY_FILE must be an owner-held non-empty regular file no larger than 16 KiB with no group or other permissions")
	}
	return nil
}
