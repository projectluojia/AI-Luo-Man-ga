...
func TestInstallReplacesVersionsAndVerifiesIntegrity(t *testing.T) {
	...
	// 回读一致。
	reloaded, err := packageio.ReadInstalled(ctx, filepath.Join(root, "demo.pkg"))
	if err != nil {
	...
	if _, err := packageio.ReadInstalled(ctx, filepath.Join(root, "demo.pkg")); err == nil {
...
func TestInstallRejectsImplicitUnixAddressOnWindows(t *testing.T) {
	...
	for _, address := range []string{"", "unix:/runtime.sock"} {
		t.Run(address, func(t *testing.T) {
			...
			if _, err := packmgr.Install(context.Background(), packageiotest.TempDir(t), source); err == nil {
...
func TestInstallRejectsEntrypointBasenameCollision(t *testing.T) {
	...
	_, err = packmgr.Install(context.Background(), packageiotest.TempDir(t), source)
...
func TestInstallSerializesSamePackagePublication(t *testing.T) {
	...
	record, err := packageio.ReadInstalled(context.Background(), filepath.Join(root, "demo.concurrent"))
	if err != nil {
