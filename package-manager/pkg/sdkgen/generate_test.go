...
func TestGenerateGoShape(t *testing.T) {
	...
	required := []string{
		...
		"func (c *Client) BusStopsSearch(ctx context.Context, input BusStopsSearchInput, options ...CallOption) (json.RawMessage, error) {",
		`return c.invoke(ctx, "campus.bus.stops.search", input, options...)`,
		`return c.invoke(ctx, "campus.bus.journeys.search", input, options...)`,
	}
...
func TestGeneratePythonShape(t *testing.T) {
	...
	required := []string{
		...
		"def bus_stops_search(client: Client, input: BusStopsSearchInput, options: Optional[CallOptions] = None) -> Any:",
		`return client._invoke("campus.bus.stops.search", input, options)`,
	}
	for _, want := range required {
		if !strings.Contains(code, want) {
