...
def _start_frame(
    pending: list[executor_pb2.ConfirmationInfo] | None = None,
...
class ConfirmationRoundTripTest(unittest.IsolatedAsyncioTestCase):
    @staticmethod
    async def _next_body(
        output: AsyncIterator[executor_pb2.ExecutorFrame], body: str
    ) -> executor_pb2.ExecutorFrame:
    ...
    def _stream(
        self,
    ) -> tuple[asyncio.Queue, AsyncIterator[executor_pb2.ExecutorFrame]]:
    ...
    async def test_approved_confirmation_is_attached_to_retry_call(self) -> None:
        ...
        model = ScriptedModel(
            lambda: _tool_call_turn(
                ToolCall("call-1", "cap_campus_bus_routes_list", '{"limit":10}')
            ),
            check,
        )
    ...
    async def test_waiting_confirmation_does_not_suppress_capability_call(self) -> None:
        ...
        call = await self._next_body(output, "capability_call")
        self.assertEqual(
            call.capability_call.confirmation_id,
            "",
        ...
        self.assertEqual(
            seen["payload"]["error"]["confirmation"]["confirmation_id"], "conf-2"
        )
    ...
    async def test_malformed_confirmation_projection_is_rejected(self) -> None:
        ...
        await requests.put(
            _start_frame(
                pending=[_confirmation("conf-4", "waiting", target_type="database")]
            )
        )
        output = ExecutorRuntime(ScriptedModel(lambda: _final_turn("不应到达"))).Run(
    ...
    async def test_confirmation_required_projection_fields_are_validated(self) -> None:
        ...
        for confirmation in (
            ...
            requests, request_iterator = self._stream()
            await requests.put(_start_frame())
            output = ExecutorRuntime(
                ScriptedModel(
                    lambda: _tool_call_turn(
                        ToolCall("call-1", "cap_campus_bus_routes_list", '{"limit":10}')
                    )
                )
            ).Run(request_iterator, None)
            await anext(output)
            call = await self._next_body(output, "capability_call")
            await requests.put(
                executor_pb2.ExecutorFrame(
