...
启动该进程，再由 Core 按安装 lock 连接（把本包运行时 ID `agent.executor` 加入
`AILUO_EXTERNAL_RUNTIMES`）；Core 不负责解释或注入本包的 Provider
环境变量。日志配置中的非法值会使进程直接退出，不会静默回退到默认值。若改为由
Core 托管启动，缺少 Provider 配置会按未就绪处理。
