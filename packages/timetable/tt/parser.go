...
// ParseWakeUpCSV 解析 WakeUp 公开脚本导出的 CSV 行布局（带 7 列表头，
// 字段允许带引号含换行）。fileName 仅用于派生默认课表名。
...
// ParseWakeUpLegacy 解析旧版 WakeUp JSON 行布局（5 行：两行头、课表名、
// 课程基表、课程明细）。
...
// FileBase 取文件名去掉目录与扩展名（不依赖 filepath：wasip1 guest 与
// 宿主平台路径规则无关，只做通用 / \ 分隔与扩展点处理）。
func FileBase(name string) string {
	...
	return strings.TrimSuffix(name, lastExt(name))
}
