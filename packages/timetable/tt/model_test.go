...
func TestParseWakeUpCSVQuotedAndLegacy(t *testing.T) {
	data, err := ParseWakeUpCSV(`name,day,startNode,endNode,teacher,location,weekMeta
"高等,数学",2,1,2,"张老师","教一,101",1-16周`, "我的,课表.csv")
...
[{"id":7,"day":5,"startNode":6,"step":2,"type":2,"teacher":"李老师","room":"教二"}]`
	legacyData, err := ParseWakeUpLegacy(legacy)
...
[{"id":"7","day":"4","startNode":"2","step":"3","type":"1","teacher":"王老师","room":"教三","startWeek":"3","endWeek":"18"}]`
	data, err := ParseWakeUpLegacy(legacy)
...
[{"id":7,"day":"9","startNode":2,"step":2,"type":2,"teacher":"李老师","room":"教二"}]`
	if _, err := ParseWakeUpLegacy(legacy); !errors.Is(err, ErrMalformedData) {
		t.Fatalf("legacy invalid row err=%v", err)
	}
}

func TestParseWakeUpCSVRejectsClassPeriodBeyondLimit(t *testing.T) {
	_, err := ParseWakeUpCSV("name,day,startNode,endNode,teacher,location,weekMeta\n高数,1,1,65,张老师,教一,1-16周", "")
...
func TestNormalizeCourseRejectsOverlongDisplayFields(t *testing.T) {
	course := Course{TimetableID: "table-1", ID: "course-1",
...
func TestParseWakeUpCSVHandlesQuotedNewline(t *testing.T) {
	...
	data, err := ParseWakeUpCSV(content, "")
	if err != nil || len(data.Courses) != 1 {
...
func TestFileBaseStripsDirectoriesAndExtensions(t *testing.T) {
	cases := map[string]string{
		"我的课表.csv":        "我的课表",
		"a/b/课表 v2.xlsx":  "课表 v2",
		`C:\Users\课表.cvs`: "课表",
		"":                "",
		".hidden":         ".hidden",
		"noext":           "noext",
	...
	for input, want := range cases {
		if got := FileBase(input); got != want {
			t.Fatalf("FileBase(%q)=%q, want %q", input, got, want)
		}
	}
}

func TestValidDocIDMatchesHostStableIDRule(t *testing.T) {
	valid := []string{"t0189d2c8", "table-1", "course.2.a", "import-12"}
	invalid := []string{"", "Table", "550e8400-e29b", "-lead", "包含中文", "space id", "trailing-", "course.2:a", "a..b"}
	for _, id := range valid {
		if !ValidDocID(id) {
			t.Fatalf("ValidDocID(%q)=false, want true", id)
		}
	}
	for _, id := range invalid {
		if ValidDocID(id) {
			t.Fatalf("ValidDocID(%q)=true, want false", id)
		}
	}
}

func TestNormalizeTimetableRejectsInvalidSourceAndID(t *testing.T) {
	if _, err := NormalizeNewTimetable(Timetable{ID: "t1", Name: "期中", Source: "mit"}); !errors.Is(err, ErrInvalid) {
		t.Fatalf("invalid source err=%v", err)
	}
	if _, err := NormalizeNewTimetable(Timetable{ID: "T1", Name: "期中", Source: SourceLocal}); !errors.Is(err, ErrInvalid) {
		t.Fatalf("uppercase id err=%v", err)
	}
	if _, err := NormalizeTimetable(Timetable{ID: "", Name: "期中", Source: SourceLocal}); !errors.Is(err, ErrInvalid) {
		t.Fatalf("empty id err=%v", err)
	}
	item, err := NormalizeNewTimetable(Timetable{Name: "  期中\t ", Source: SourceLocal})
	if err != nil || item.Name != "期中" || item.ID != "" {
		t.Fatalf("allowEmptyID item=%#v err=%v", item, err)
	}
}

func TestCourseJSONRoundTripKeepsContractFields(t *testing.T) {
	course, err := NormalizeCourse(Course{TimetableID: "table-1", ID: "c1", Title: "高数",
		Weekday: 2, ClassFrom: 1, ClassTo: 2, Weeks: []int{2, 1}, Instructor: "张老师"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if decoded.ID != "c1" || decoded.TimetableID != "table-1" || !sameInts(decoded.Weeks, []int{1, 2}) {
		t.Fatalf("round trip decoded=%#v", decoded)
	}
	if strings.Contains(string(payload), `"title_raw"`) {
		t.Fatalf("empty optional fields must be omitted: %s", payload)
	}
}
