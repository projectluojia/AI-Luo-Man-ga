// package wx 提供天气 isolated provider 的领域模型：直连 Open-Meteo、
// 小米天气与 AccuWeather，规范化逐小时/AQI/预警，并带进程内缓存与有界重试。
// 错误统一映射到内核公开错误码闭集（guest 不发明私有码）。
package wx
...
// 内核公开错误码闭集（internal/kernel/publicerror capabilityCodeTable）。
// isolated provider 无法 import 内核内部包，因此这里以字面量闭式复刻；
// CI 的 code-drift 测试保证两侧永不漂移。
const (
	CodeInvalidArguments   = "invalid_arguments"
	CodeDataUnavailable    = "data_unavailable"
	CodeDataIncomplete     = "data_incomplete"
	CodeDataUntrusted      = "data_non_authoritative"
	CodeDataExpired        = "data_expired"
	CodeCapabilityFailed   = "capability_failed"
	CodeDeadlineExceeded   = "deadline_exceeded"
	CodeRuntimeUnavailable = "runtime_unavailable"
)

// Error 是带稳定码的领域错误；gRPC server 层把它直接映射进
// InvokeResponse.ErrorCode（稳定 ID 字符集）。
type Error struct {
	Code      string
	Message   string
	Retryable bool
...
func dataUnavailable(format string, args ...any) *Error {
	return &Error{Code: CodeDataUnavailable, Message: fmt.Sprintf(format, args...), Retryable: true, cause: ErrDataUnavailable}
}

func dataIncomplete(format string, args ...any) *Error {
	return &Error{Code: CodeDataIncomplete, Message: fmt.Sprintf(format, args...), cause: ErrDataIncomplete}
}

func dataUntrusted(format string, args ...any) *Error {
	return &Error{Code: CodeDataUntrusted, Message: fmt.Sprintf(format, args...), cause: ErrDataUntrusted}
}

func dataExpired(format string, args ...any) *Error {
	return &Error{Code: CodeDataExpired, Message: fmt.Sprintf(format, args...), Retryable: true, cause: ErrDataExpired}
}
...
var (
	...
	// 数据治理哨兵错误：构造函数生成的 *Error 用 Is 语义关联对应哨兵，
	// 测试与调用方据此分类，不必比较错误字符串。
	ErrDataIncomplete  = errors.New("weather data is incomplete")
	ErrDataUntrusted   = errors.New("weather data is not authoritative")
	ErrDataUnavailable = errors.New("weather data is unavailable")
	ErrDataExpired     = errors.New("weather data has expired")
)
...
func (q *LocationQuery) NormalizeAndValidate() error {
	...
	if q.Hours < 1 || q.Hours > MaxHours {
		return invalidArguments("hours must be between 1 and %d", MaxHours)
	}
...
func (q *GeocodeQuery) NormalizeAndValidate() error {
	...
	if q.Place == "" || utf8.RuneCountInString(q.Place) > MaxPlaceRunes {
		return invalidArguments("place is required and must be at most %d characters", MaxPlaceRunes)
	}
	return nil
}
...
func (r *LookupRequest) NormalizeAndValidate() error {
	...
	default:
		return invalidArguments("unknown weather source %q", r.Source)
	}
	if r.Hours == 0 {
	...
	if (r.Latitude == nil) != (r.Longitude == nil) {
		return invalidArguments("latitude and longitude must be provided together")
	}
	if r.Latitude != nil {
	...
	if r.Place != "" && utf8.RuneCountInString(r.Place) > MaxPlaceRunes {
		return invalidArguments("place must be at most %d characters", MaxPlaceRunes)
	}
...
// Govern 是 fail-closed 的数据治理：新鲜度元数据不完整、非权威或已过期都拒绝
// 作为事实返回。
func (s DataStatus) Govern(now time.Time) (DataStatus, error) {
	if s.Source == "" || s.Provider == "" || s.Revision == "" || s.FetchedAt.IsZero() || s.ValidUntil.IsZero() ||
		...
		return DataStatus{}, dataIncomplete("weather data status is incomplete")
	}
	if !s.Authoritative || !s.Complete {
		return DataStatus{}, dataUntrusted("weather data is not authoritative")
	}
	if !now.Before(s.ValidUntil) {
		return DataStatus{}, dataExpired("weather data has expired")
	}
...
func validateCoordinates(lat, lon float64) error {
	if math.IsNaN(lat) || math.IsInf(lat, 0) || lat < -90 || lat > 90 {
		return invalidArguments("latitude is out of range")
	}
	if math.IsNaN(lon) || math.IsInf(lon, 0) || lon < -180 || lon > 180 {
		return invalidArguments("longitude is out of range")
	}
	return nil
}
...
func marshalResult(value any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, &Error{Code: CodeCapabilityFailed, Message: "encode weather result"}
	}
	return encoded, nil
}
...
func parseRFC3339(value string) (time.Time, error) {
	...
	if value == "" {
		return time.Time{}, dataIncomplete("weather timestamp is empty")
	}
	...
	return time.Time{}, dataIncomplete("weather timestamp is invalid")
}
