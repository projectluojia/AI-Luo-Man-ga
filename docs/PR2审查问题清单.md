...
## PR2-003：平台消息与 Echo 幂等键缺少入口命名空间
...
- 两个平台都产生消息标识 `123`，会在同一 App 内冲突；
- Web 客户端幂等键与 QQ 消息标识相同，可能错误命中同一个 Echo；
- 不同用户使用同一键和相同文本时，会重放第一个用户的 Echo；
- 不同用户使用同一键但文本不同时，消息已经入库，随后 Echo 创建返回冲突；
- ingress 返回当前 Intake 的 `session_id/message_id`，但 `echo_id` 可能来自另一个入口的旧请求。
